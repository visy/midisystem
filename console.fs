...
void main()
{
    ...
    if (time > 56500) timespeed=2.8f;

    ypos = height;
    if (time > 18000) ypos = height-(time-18000)*(0.006*timespeed);

    if (ypos < 0) ypos = 0;

    ypos2 = height;
    if (time > 13500) ypos2 = height-(time-13500)*(0.005*timespeed);

    if (ypos < 0) ypos = 0;
    if (ypos2 < 0) ypos2 = 0;

    if (time > 36000) ypos = ypos2;
    ...
    if (gl_FragCoord.y > ypos2)
    {
        ...
        if ((color.r + color.b + color.g)/3.0 > 0.3 && col.g > 0.2) { color.rgb = vec3(0,0,0); }

        if ((color.r + color.b + color.g)/3.0 < 0.8) color.rgb = vec3(0,0,0);
        gl_FragColor = color;
    }
}
